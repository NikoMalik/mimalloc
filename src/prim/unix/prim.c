...
size_t get_page_size_from_auxv(void) {

    char **envp = mi_get_environ();
...
static long get_phys_pages_syscall(void) {
    struct sysinfo info;
    long ret = syscall1(SYS_sysinfo, &info);
    if (ret != 0)
        return 0;
...
void _mi_prim_mem_init(mi_os_mem_config_t *config) {
    long psize = get_page_size_from_auxv();
    if (psize > 0) {
        config->page_size = (size_t)psize;
        config->alloc_granularity = (size_t)psize;
        long pphys = get_phys_pages_syscall();
        if (pphys == 0) {
            goto n;
        }
...
int _mi_prim_free(void *addr, size_t size) {
    if (size == 0)
        return 0;
    long r = munmap(addr, size);
    return mi_os_error_from(r);
}
...
#if defined(__sun)
    long res = madvise((caddr_t)addr, size, advice); // Solaris needs cast (issue #520)
#elif defined(__QNX__)
    long res = posix_madvise(addr, size, advice);
#else
    long res = madvise(addr, size, advice);
#endif
    return mi_os_error_from(res);
}
            ...
            if (large_only || lflags != flags) {
                ...
                *is_large = true;
                ...
                if (p == NULL && (lflags & MAP_HUGE_1GB) == MAP_HUGE_1GB) {
                    ...
                    if (large_only) {
                        _mi_warning_message("unable to allocate huge (1GiB) page, trying 2MiB instead (error: %d)\n", mi_os_error_from((long)p));
                    }
...
int _mi_prim_alloc(void *hint_addr, size_t size, size_t try_alignment, bool commit, bool allow_large, bool *is_large, bool *is_zero, void **addr) {
    ...
    *addr = unix_mmap(hint_addr, size, try_alignment, protect_flags, false, allow_large, is_large);
    return (*addr != NULL ? 0 : mi_os_error_from((long)*addr));
}
...
int _mi_prim_commit(void *start, size_t size, bool *is_zero) {
    ...
    *is_zero = false;
    long r = mprotect(start, size, (PROT_READ | PROT_WRITE));

    if (r < 0) {
        int err = mi_os_error_from(r);
...
// on macOS can use MADV_FREE_REUSABLE (but we disable this for now as it seems slower)
  ...
  // fall through
    ...
    while ((err = unix_madvise(start, size, oadvice)) != 0 && mi_os_error_from(err) == 11) {
        /*retry*/
    };
    if (err != 0 && mi_os_error_from(err) == 22 && oadvice == MADV_FREE) {
...
int _mi_prim_protect(void *start, size_t size, bool protect) {
    long r = mprotect(start, size, protect ? PROT_NONE : (PROT_READ | PROT_WRITE));
    if (r < 0) {
        unix_mprotect_hint(mi_os_error_from(r));
    }
    return (int)r;
}
...
static long mi_prim_mbind(void *start, unsigned long len, unsigned long mode, const unsigned long *nmask, unsigned long maxnode, unsigned flags) {
    return syscall6(SYS_mbind, start, len, mode, nmask, maxnode, flags);
}
#else
static long mi_prim_mbind(void *start, unsigned long len, unsigned long mode, const unsigned long *nmask, unsigned long maxnode, unsigned flags) {
    MI_UNUSED(start);
...
size_t _mi_prim_numa_node(void) {
    ...
    long r = syscall3(SYS_getcpu, &ncpu, &node, NULL);
    if (r < 0)
        return 0;
    return node;
#else
...
size_t _mi_prim_numa_node_count(void) {
    ...
    for (node = 0; node < 256; node++) {
        ...
        if (mi_prim_access(buf, 4) != 0) // R_OK
            break;
    }
    return (node + 1);
}
...
void _mi_prim_process_info(mi_process_info_t *pinfo) {
    ...
    if (getrusage(RUSAGE_SELF, &ru) == 0) {
        pinfo->utime = timeval_secs(&ru.ru_utime);
        pinfo->stime = timeval_secs(&ru.ru_stime);
        pinfo->page_faults = ru.ru_majflt;
        pinfo->peak_rss = ru.ru_maxrss * 1024; // Linux/BSD: ru_maxrss in KiB
    } else {
        // fallback to zeros
        pinfo->utime = 0;
        pinfo->stime = 0;
...
static inline void _mi_prim_out_stderr(const char *msg) {
    const char *p = msg;
    size_t len = 0;
    while (p[len] != '\0')
        len++;
    sys_write(2, p, len); // 2 = stderr
}
...
#define GRND_NONBLOCK (1)
    ...
    if (mi_atomic_load_acquire(&no_getrandom) == 0) {
        long ret = syscall3(SYS_getrandom, buf, buf_len, GRND_NONBLOCK);
        if (ret >= 0)
            return (buf_len == (size_t)ret);
        if (mi_os_error_from(ret) != ENOSYS)
            return false;
    ...
    while (count < buf_len) {
        ...
        if (ret <= 0) {
            if (ret < 0 && mi_os_error_from(ret) != 11 && mi_os_error_from(ret) != 4) // 11= EAGAIN 4 = EINTR
                break;
        } else {
            count += ret;
        }
