...
static inline int _mi_isspace(char c) {
    return c == ' ' || c == '\t' || c == '\n' || c == '\r' || c == '\v' || c == '\f';
}

static inline int _mi_isdigit(int c) {
    return (unsigned)c - '0' <= 9;
}

static inline int _mi_isalpha(int c) {
    return ((unsigned)c | 32) - 'a' < 26;
}

static inline int _mi_digit_value(char c) {
    if (_mi_isdigit(c))
        return c - '0';
    if ((unsigned)(c | 32) - 'a' < 6)
        return (c | 32) - 'a' + 10;
    return -1;
}
...
long _mi_strtol(const char *nptr, char **endptr, int base) {
    ...
    const char *s = nptr;
    while (_mi_isspace((unsigned char)*s))
        ++s;
    ...
    return negative ? -(long)acc : (long)acc;
}

long long _mi_strtoll(const char *nptr, char **endptr, int base) {
    if (endptr)
        *endptr = (char *)nptr;
    if (!nptr)
        return 0;
    ...
    unsigned long long cutoff = negative ? -(unsigned long long)LLONG_MIN : (unsigned long long)LLONG_MAX;
    ...
    while (1) {
        ...
        if (acc > cutoff || (acc == cutoff && (unsigned long long)digit > cutlim)) {
            overflow = true;
        }
    ...
    return negative ? -(long long)acc : (long long)acc;
}
    ...
    while (true) {
        ...
        if (c != '%') {
            ...
            if (c >= '1' && c <= '9') {
                ...
                if (c == 0)
                    break; // extra check due to while
            }
            if (c == 'z' || c == 't' || c == 'L') {
                numtype = c;
                MI_NEXTC();
