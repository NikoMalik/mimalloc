...
static void mi_stats_merge_from(mi_stats_t *stats) {
    if (stats != &_mi_stats_main) {
        ...
        _mi_memset(stats, 0, sizeof(mi_stats_t));
    }
}
...
void mi_stats_reset(void) mi_attr_noexcept {
    ...
    _mi_memset(&_mi_stats_main, 0, sizeof(mi_stats_t));
...
static void mi_heap_buf_print_count_bin(mi_heap_buf_t *hbuf, const char *prefix, mi_stat_count_t *stat, size_t bin, bool add_comma) {
    ...
    const size_t pagesize = (binsize <= MI_SMALL_OBJ_SIZE_MAX ? MI_SMALL_PAGE_SIZE : (binsize <= MI_MEDIUM_OBJ_SIZE_MAX ? MI_MEDIUM_PAGE_SIZE :
